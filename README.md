...
- **Material quantizer: functional.** *Liero > Quantize by Material…* on an RGB image: scans top-level layers/groups for material names (override per layer; a layer can also cover a material **group** like Dirts or Backgrounds — slots of all members are reusable, new slots get the group's primary material), per-material color counts (defaulting to the slots that material owns in the base palette), base palette (classic default or any toolkit GIMP palette — materials/ANIM recovered from entry names), keep-worm-slots and unique-colors options. *Quantize (preview)* shows the planned palette on the grid (allocated slots selected; select swatches and *Assign material* to override the table — edits feed the plan and the next run) and the quantized pixels on a zoomable canvas — view as quantized colors, **material mask** (flat color per material) or **bad pixels** (isolated-material pixels highlighted and counted, e.g. a lone BG pixel inside rock), optionally filtered to one material. *Apply to layers* quantizes each assigned layer/group via the sibling `custom:palette-quantize` GEGL op — **non-destructively as editable layer effects by default**, or baked in (python fallback) when unchecked; the XCF stays RGB/editable but palette-correct, one undo step reverts. *Create indexed image* produces a new image (exact indices via an internally-written indexed PNG — GIMP's own convert loses uniquify nudges) plus the forked palette resource. Dialog settings persist in an image parasite (saved into the XCF), so re-quantizing is two clicks. Allocation: reuse close same-material slots first, then the 188-235 pool (only slots still UNDEF).
- **OpenLiero level export: functional.** *Liero > Export OpenLiero Level…* on an RGB image (the display/map) writes an [OpenLiero](https://github.com/openliero/openliero) **MODERNLV** `.lev`. Pick a separate **indexed** image of matching size as the material mask (e.g. the Quantize-by-Material output); optionally add a `ramps.json` and an anim layer (R=ramp 1-based, G=phase). The dialog previews **Display / Material mask / Animation** (▶ live ramp cycling). The writer is verified byte-identical to OpenLiero's own `tools/lev_gen.py` and its engine loader (`level.cpp`), and OpenLiero's default material table is identical to this toolkit's. POWERLEVEL/classic-only levels are out of scope — every exported level carries a MODERNLV layer.

### Where exported levels (and OpenLiero settings) live

OpenLiero keeps levels and config under its `SDL_GetPrefPath("openliero","openliero")` data root. Drop an exported `.lev` anywhere in that root (the in-game picker walks subdirectories):

| Platform | Data root |
|---|---|
| Linux (native) | `~/.local/share/openliero/openliero/` |
| Linux (**Flatpak** `io.github.openliero.openliero`) | `~/.var/app/io.github.openliero.openliero/data/openliero/openliero/` |
| macOS | `~/Library/Application Support/openliero/openliero/` |
| Windows | `%APPDATA%\openliero\openliero\` |

Within that root, OpenLiero stores game settings in `Setups/liero.cfg` and key/control bindings in `Profiles/*.toml`. Overrides: `OPENLIERO_DATADIR` (data root), a portable install (`portable.txt` / `--config-root`).
