...
- **Material quantizer: functional.** *Liero > Quantize by Material…* on an RGB image: scans top-level layers/groups for material names (override per layer in the dialog), per-material color counts, base palette (classic default or any toolkit GIMP palette — materials/ANIM recovered from entry names), keep-worm-slots and unique-colors options. *Quantize (preview)* shows the planned palette on the grid (allocated slots selected) and the quantized pixels on a canvas with reuse/pool stats; *Create indexed image* produces a new image (exact indices via an internally-written indexed PNG — GIMP's own convert loses uniquify nudges) plus the forked palette resource. Allocation: reuse close same-material slots first, then the 188-235 pool (only slots still UNDEF).
